  ...
  return (
    <div className="px-4 my-20 sm:px-20 ">
      <div className="mb-4 flex flex-col justify-center items-center gap-8 sm:flex-row sm:justify-between">
        <h1 className="font-semibold text-center sm:text-left text-my-green-800 text-2xl">
          Tasks List 📋
        </h1>

        {todos.every((todo) => todo.isCompleted) && todos.length > 0 ? (
          <p className="text-center text-my-green-800 rounded-lg font-extrabold">
            ALL TASKS COMPLETED
          </p>
        ) : (
          <div className="flex flex-row justify-center items-center gap-4 flex-wrap">
            <p className="text-center text-my-green-800 rounded-lg font-extrabold">
              Total Todos :
              <span className="rounded-lg px-2 py-1 ml-1 text-my-green-100 bg-my-green-800">
                {todos.length}
              </span>
            </p>
            <p className="text-center text-my-green-800 rounded-lg font-extrabold">
      ...
      <ul>
        {todos.length === 0 ? (
          <li className="mt-20 font-thin text-xl text-my-green-800 text-center">
            No task added
          </li>
        ) : (
          todos.map((todo) => (
            <li
              ...
              className={`${
                todo.isCompleted
                  ? "bg-[rgba(41,133,127,0.2)] line-through text-my-green-800"
                  : "bg-white"
              ...
              {todo.edit ? (
                <div className="flex-1 flex flex-row justify-between items-center">
                  <input
                    ...
                    className="rounded-lg border-[2px] border-solid border-my-text-my-green-800 text-left flex-1 px-3  transition-all ease-in-out duration-[0.3s] hover:bg-[rgba(41,133,127,0.2)]"
                    value={value}
                  ...
                  <RiSave2Fill
                    ...
                    className=" p-1 text-[rgba(41,133,127,0.7)] transition-all ease-in-out duration-300 active:scale-50 active:text-my-green-800"
                    size={"2rem"}
                    title="Save"
                  />
                </div>
